...
func MAMAFAMA(vals []float64, fastLimit, slowLimit float64, which int) []float64 {
	...
	for i := 0; i < len(vals); i++ {
		...
		s := (4*vals[i] + 3*vals[i-1] + 2*vals[i-2] + vals[i-3]) / 10
		smooth[i] = s
		d := (0.0962*s + 0.5769*smooth[i-2] - 0.5769*smooth[i-4] - 0.0962*smooth[i-6]) * (0.075*period[i-1] + 0.54)
		detrender[i] = d
		...
		jI := (0.0962*detrender[i-3] + 0.05769*detrender[i-5] - 0.5769*detrender[i-7] - 0.0962*detrender[i-9]) * (0.075*period[i-1] + 0.54)
		...
		i2 = 0.2*i2 + 0.8*is[i-1]
		is[i] = i2
		q2 = 0.2*q2 + 0.8*qs[i-1]
		q2s[i] = q2

		// Homodyne Discriminator
		re := i2*is[i-1] + q2*q2s[i-1]
		res[i] = re
		im := i2*q2s[i-1] - q2*is[i-1]
		ims[i] = im
